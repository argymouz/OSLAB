...
static void lunix_protocol_update_sensors(struct lunix_protocol_state_struct *state, struct lunix_sensor_struct *lunix_sensors)
{
	...
	{
		nodeid = uint16_from_packet(&state->packet[NODE_OFFSET]); // take the part of the packet that has arrived which corresponds to the nodeid
		batt = uint16_from_packet(&state->packet[VREF_OFFSET]); // in order to understand which node these measurements come from
